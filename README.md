...
- Создать свой билдер, наследуемый от `fermi::VFermiPropertiesBuilder`, который заполнит таблицу в методе `Build`.
Для инициализации таблицы с помощью билдера следует добавить следующие строчку:

```c++
fermi::NucleiProperties props(*Your builder here*);
```
