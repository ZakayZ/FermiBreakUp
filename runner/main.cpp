...
void CalculateFragments(
  ...
  G4FermiFloat step = 0.2,
  size_t tests = 1e4)
{
  auto model = G4FermiBreakUp(std::make_unique<G4FermiSimpleCache<G4FermiNucleiData, G4FermiFragmentSplits>>());
  ...
  for (G4FermiFloat energyNucleon = 0.; energyNucleon <= 10.; energyNucleon += step) {
    ...
    for (size_t i = 0; i < tests; ++i) {
      auto vec = G4FermiLorentzVector(0, 0, 0, G4FermiNucleiProperties()->GetNuclearMass(mass, charge) + additionalEnergy);
...
void CalculateMomentum(
  ...
  const std::string& dumpName,
  G4FermiFloat energy,
  const G4FermiVector3& momentum,
  size_t tests = 1e4)
{
  auto model = G4FermiBreakUp(std::make_unique<G4FermiSimpleCache<G4FermiNucleiData, G4FermiFragmentSplits>>());
  std::ofstream out(dumpName);
  auto vec = G4FermiLorentzVector(momentum.x(), momentum.y(), momentum.z(),
