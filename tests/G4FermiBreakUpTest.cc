...
namespace {
  ...
  std::unique_ptr<G4FermiVCache<G4FermiNucleiData, G4FermiFragmentSplits>> GetCache(const G4FermiStr& type) {
...
float CalculateFragmentCount(
  ...
  const G4FermiVector3& vec,
  G4FermiFloat energyPerNucleon,
  size_t tests,
  std::unique_ptr<G4FermiVCache<G4FermiNucleiData, G4FermiFragmentSplits>>&& cache = nullptr)
{
  ...
  const auto totalEnergy = std::sqrt(std::pow(G4FermiNucleiProperties()->GetNuclearMass(mass, charge) + energy, 2) + vec.mag2());
...
TEST_P(G4FermiConfigurationsFixture, CarbonDecay) {
  ...
  ASSERT_NEAR(1, CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 0 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);
  ASSERT_NEAR(1, CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 0.25 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);
  ASSERT_NEAR(1, CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 0.5 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);
  ASSERT_NEAR(1, CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 0.7 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);

  // Carbons should break into 3 parts [1, 1.4]
  ASSERT_NEAR(3, CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 1 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);
  ASSERT_NEAR(3, CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 1.25 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);
  ASSERT_NEAR(3, CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 1.4 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);

  // Carbons should break into less than 3 parts [1.5, 3.5]
  ASSERT_LE(CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 1.5 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 3);
  ASSERT_LE(CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 2 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 3);
  ASSERT_LE(CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 3 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 3);
  ASSERT_LE(CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 3.5 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 3);

  // Carbons should break into more than 3 parts [5, ...]
  ASSERT_GE(CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 5 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 3);
  ASSERT_GE(CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 7 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 3);
  ASSERT_GE(CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 9 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 3);
  ASSERT_GE(CalculateFragmentCount(12_m, 6_c, {0, 0, 0}, 20 * CLHEP::MeV, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 3);
}
...
TEST_P(G4FermiConfigurationsFixture, UnstableNuclei) {
  ...
  for (int i = 2; i <= 16; ++i) {
    ASSERT_NEAR(i, CalculateFragmentCount(G4FermiAtomicMass(i), G4FermiChargeNumber(i), {0, 0, 0}, 0, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);
  }

  // neutron should break up
  for (int i = 2; i <= 16; ++i) {
    ASSERT_NEAR(i, CalculateFragmentCount(G4FermiAtomicMass(i), 0_c, {0, 0, 0}, 0, RUNS, GetCache(G4FermiConfigurationsFixture::GetParam())), 0.01);
  }
}
  ...
  for (int t = 0; t < TRIES; ++t) {
    ...
    for (size_t i = 0; i < RUNS; ++i) {
      ...
      for (const auto& fragment : particles) {
        fragmentsMassSum = G4FermiAtomicMass(G4FermiUInt(fragmentsMassSum) + G4FermiUInt(fragment.GetAtomicMass()));
        fragmentsChargeSum = G4FermiChargeNumber(G4FermiUInt(fragmentsChargeSum) + G4FermiUInt(fragment.GetChargeNumber()));
      }
