...
namespace {
  ...
  G4FermiFloat CoulombBarrier(const G4FermiFragmentVector& split) {
    ...
    static const G4FermiFloat coef = (3. / 5.) * (CLHEP::elm_coupling / R0) * std::cbrt(1. / (1. + Kappa));
  ...
  inline size_t Factorial(const size_t n) {
    ...
    for (G4FermiUInt i = 2; i <= n; ++i) { factorial *= i; }
    return factorial;
  }
  ...
  G4FermiFloat ConfigurationFactor(const G4FermiFragmentVector& split) {
    ...
    std::transform(split.begin(), split.end(), masses.begin(), std::mem_fn(&G4FermiFragment::GetAtomicMass));
  ...
  G4FermiFloat ConstFactor(G4FermiAtomicMass atomicMass, size_t fragmentsCount) {
    static const G4FermiFloat COEF = std::pow(R0 / CLHEP::hbarc, 3) * Kappa * std::sqrt(2.0 / CLHEP::pi) / 3.0;
...
G4FermiFloat G4FermiSplitter::DecayWeight(const G4FermiFragmentVector& split, G4FermiAtomicMass atomicMass, G4FermiFloat totalEnergy) {
...
namespace {
  ...
  G4FermiFragmentSplits PossibleSplits(const G4FermiPartition& massPartition, const G4FermiPartition& chargePartition) {
    ...
    for (size_t fragmentIdx = 0; fragmentIdx < fragmentCount; ++fragmentIdx) {
      const auto fragmentRange = fragmentPool.GetFragments(G4FermiAtomicMass(massPartition[fragmentIdx]),
                                                           G4FermiChargeNumber(chargePartition[fragmentIdx]));
      // no remainder here!
...
void G4FermiSplitter::GenerateSplits(G4FermiNucleiData nucleiData, G4FermiFragmentSplits& splits) {
  ...
  for (G4FermiUInt fragmentCount = 2; fragmentCount <= maxFragmentsCount; ++fragmentCount) {
    ...
    for (auto& massPartition : G4FermiIntegerPartition(nucleiData.atomicMass, fragmentCount, 1)) {
      for (auto& chargePartition : G4FermiIntegerPartition(nucleiData.chargeNumber, fragmentCount, 0)) {
