...
namespace fbu {
  ...
  public:
    G4FermiStreamLogger(std::ostream& stream)
      : stream_(stream)
    {
    }
  ...
  public:
    ...
    G4FermiLogger(std::shared_ptr<G4FermiILogger>&& impl, const G4FermiLogLevel level = G4FermiLogLevel::TRACE)
      : impl_(std::move(impl))
      , level_(level)
    {
    }

    G4FermiLogger(const std::shared_ptr<G4FermiILogger>& impl, const G4FermiLogLevel level = G4FermiLogLevel::TRACE)
    ...
    static G4FermiLogger Default() {
      return G4FermiLogger(GlobalImpl, GlobalLevel);
    }

    static inline std::shared_ptr<G4FermiILogger> GlobalImpl = std::make_shared<G4FermiStreamLogger>(std::cerr);
